...
import { SetRecipesType } from "../types";

type HeaderProps = {
    setRecipes: SetRecipesType;
}

const Header = ( { setRecipes }: HeaderProps ) => {
        ...
        try {
            ...
            if(data.results.length === 0) {
                ...
                const formattedData = data.results.map((recipe: any) => ({
                    ...
                    instructions: recipe.analyzedInstructions.flatMap((instruction: any) => 
    ...
    return ( 
        <div className="header">
            ...
            <form className="search" onSubmit={handleSearch}>
                <input type="text" id="search" name="search" placeholder="bacon, sugar, flour" value={searchTerm} onChange={handleSearchTermChange} />
                <button type="submit">Search</button>
                <br />
                <input type="checkbox" id="vegetarian" name="vegetarian" checked={isVegetarian} onChange={handleVegChange} />
                <label htmlFor="vegetarian"> Vegetarian </label>
                <input type="checkbox" id="gluten-free" name="gluten-free" checked={isGlutenFree} onChange={handleGlutenChange} />
